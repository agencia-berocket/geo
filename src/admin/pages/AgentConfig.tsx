...
const fileExplanations: Record<string, string> = {
  ...
  'MAPA.md': 'O guia descritivo dos arquivos e dos caminhos de sandbox locais que o agente tem permissão de acessar.',
...
const fileLabels: Record<string, string> = {
  ...
  'MAPA.md': 'MAPA.md (Mapa do Sandbox)',
...
export default function AgentConfig() {
  ...
  const handleGitSync = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) {
        setMessage(data.message || 'Sincronizado com sucesso!');
        setMessageIsError(false);
      } else {
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex justify-between items-start flex-wrap gap-4">
        ...
        >
          {syncing ? (
            <><IconHourglass className="w-3.5 h-3.5" /> Enviando ao GitHub...</>
          ) : (
            <><IconUpload className="w-3.5 h-3.5" /> Enviar e Sincronizar com GitHub (Push)</>
          )}
        </button>
      </div>
      ...
      <div className="flex border-b border-zinc-200 gap-6">
        <button
          ...
          className={`pb-4 text-sm font-semibold transition-all cursor-pointer border-b-2 flex items-center gap-2 ${
            activeMode === 'general' ? 'border-zinc-950 text-zinc-950' : 'border-transparent text-zinc-400 hover:text-zinc-600'
          }`}
        >
          <IconFolder className="w-4 h-4" /> Documentos de Arquitetura Geral
        </button>
        <button
        ...
        <div className="lg:col-span-8 space-y-6">
          ...
          <div className="bg-zinc-950 text-white p-5 rounded-2xl shadow-lg border border-zinc-800 space-y-2">
            <h3 className="font-display font-bold text-sm text-zinc-100 flex items-center gap-2">
              <IconClipboard className="w-4 h-4" /> Para que serve este arquivo?
            </h3>
